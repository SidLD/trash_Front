...
interface CustomPieChart2Props {
  onClick: () => void;
...
export const CustomPieChart2: React.FC<CustomPieChart2Props> = ({ onClick, data1, data2, month1, month2, colors }) => {
  return (
    <Card onClick={onClick} className="transition-all cursor-pointer hover:shadow-lg w-full max-w-[700px]">
      <CardHeader>
        <CardTitle>Reasons for Food Waste</CardTitle>
      </CardHeader>
      <CardContent>
        <ChartContainer
          config={{
            reasons1: {
              label: `Reasons (${month1})`,
              color: "hsl(var(--chart-1))",
            },
            reasons2: {
              label: month2 ? `Reasons (${month2})` : "",
